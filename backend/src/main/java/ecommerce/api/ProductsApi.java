...
public class ProductsApi {
	...
	public Response selectOrd(@PathParam("filterField") String filterField,
			@PathParam("filterValue") String filterValue, @PathParam("sortField") String sortField,
			@PathParam("sortDir") String sortDir) throws EcommerceException {
	...
	@Produces({ "image/jpg" })
	@Path(value = "/getImage")
	public Response getImage(ProductsDto pdto) throws IOException, EcommerceException {
		byte[] bytes = new ProductsDao().getUrlImage(pdto.getUrl());
		if (bytes == null) {
			return Response.status(Status.NOT_FOUND).build();
		}
	...
	public Response uploadFile(
		@FormDataParam("file") InputStream uploadedInputStream) throws EcommerceException{
		InputStream inputStream = ProductsApi.class.getResourceAsStream("/pathUpload.prop");
		...
		if (inputStream != null) {
			try {
				...
				path = (String) newProperties.get("save");
			} catch (IOException e) {
				throw new EcommerceException(e.getMessage());
			}
		}
		String uploadedFileLocation = path + "alienware.jpg";
